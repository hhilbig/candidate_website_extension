# Codebook: U.S. House (2018–2024) and Senate (2002–2024) candidate websites
...
| `on_ballot` | Whether the candidate matches a Democratic or Republican general election return |
| `stage` | `2` for candidates in general elections; missing otherwise |
| `data_source` | `general_wayback` for ballot candidates; `election_year_wayback` otherwise |
| `candidate_year_stage` | Candidate/year/stage key compatible with ICPSR 226001; missing off ballot |
...
| `n_tags` | Number of visible text components delimited by `#+#` before the ICPSR 226001 text filter |
| `n_clean_tags` | Number of those components retained by the ICPSR 226001 text filter |
...
| `urlkey` | Campaign website URL represented by the selected snapshot |
...
|---|---|
| `candidate_icpsr` | Candidate name in a display form compatible with ICPSR 226001 |
| `name_key` | Normalized name compatible with ICPSR 226001 |
...
| `icpsr_n_tags*`, `icpsr_n_clean_tags*` | Mean visible text component counts before and after the ICPSR 226001 text filter |
...
| `icpsr_max_pages_1day`, `icpsr_share_pages_runaway`, `icpsr_runaway_flag` | Diagnostics for unusually dense crawls within one day |
| `icpsr_topic_<topic>` | Share assigned to the named topic across all valid pages |
| `icpsr_topic_<topic>_home` | Share assigned to the named topic using only the homepage |
| fields ending `_home` | Version of the base measure using only the homepage |
...
Topic shares are proportions that measure emphasis, not policy position. They
are missing when no valid text supports the measure. Counts are nonnegative.
Lexical diversity measures are missing when the required text is absent.
...
| `dwnom1`, `dwnom2` | DW-NOMINATE dimensions linked through DIME; missing for candidates who have not held office |
| `seat`, `dime_district`, `dime_ico` | DIME seat, district, and incumbency fields |
| `lname`, `fname` | DIME normalized name components |
| `candidate_icpsr`, `name_key`, `party_full`, `district_id` | Display fields compatible with ICPSR 226001 |
| `general_votes` | Votes in the matched general election return; missing off ballot |

## `release_roster.csv`

One row is one included candidate-year with a URL attempt. The file has 9,848
...
| `universe_source` | FEC record for the same year or reviewed ballot override |
...
| `on_ballot` | Match to a general election ballot |
| `general_votes` | Votes in the general election; missing off ballot |
